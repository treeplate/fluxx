...
class Birch extends AI {
  ...
  void complyWithLimits(GameInterface game, int handLimit, int keeperLimit) {
    ...
    List<Card> keepers = game.state.thisPlayer.keepers.toList();
  ...
  int Function(Card, Card) compareCards(GameInterface game) => (Card a, Card b) {
    return (scoreCard(b, game) - scoreCard(a, game));
  };

  int scoreCard(Card card, GameInterface game) {
    if (card is KeeperCard) {
      return scoreGoal(game.state.goal, game, card) - scoreGoal(game.state.goal, game, null); // TODO: score under goals a) in hand b) on table
    }
    if (card is GoalCard) {
      return scoreGoal(card, game, null) - scoreGoal(game.state.goal, game, null);
    }
  ...
  int scoreGoal(GoalCard goal, GameInterface game, KeeperCard newKeep) {
    ...
    if (goal is EatGoal) {
      for (KeeperCard keeper in goal.foods) {
        if (game.state.thisPlayer.hands.contains(keeper) && newKeep != keeper) {
          runningTotal -= 1;
        }
      ...
      if (game.state.thisPlayer.hands.contains(goal.eater) && newKeep != goal.eater) {
        runningTotal += 1;
      }
      if (game.state.thisPlayer.keepers.contains(goal.eater) || newKeep == goal.eater) {
        runningTotal += 2;
      }
      return runningTotal;
    }
    if (goal is ButNotGoal) {
      if (game.state.thisPlayer.hands.expand((x) => x).contains(goal.yes) && newKeep != goal.yes) {
        runningTotal += 1;
      }
      if (game.state.thisPlayer.keepers.contains(goal.yes) || newKeep == goal.yes) {
        runningTotal += 2;
      }
    ...
    if (goal is! SimpleGoal && goal is! NOfGoal) throw UnimplementedError("type of goal called a '${goal.runtimeType}'");
  ...
  void play(GameInterface game) {
    while (game.state.drawnThisTurn < game.state.drawRule && !game.state.deckEmpty) {
      game.draw();
    }
    List<Card> hand = game.state.thisPlayer.hands.first.toList();
    while (game.state.playedThisTurn < game.state.playRule || game.state.playRule == 0) {
